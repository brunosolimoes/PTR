...
void atualizarValor(double *valor, double referencia, double parametro) {
    *valor = parametro * (referencia - *valor);
}
...
void *simulacaoModeloX(void *arg) {
  ...
  FILE *arquivoLog = inicializarRegistro("registro_ymx.txt", "Tempo, ymx");
  if (!arquivoLog) {
    return NULL;
  }
...
void *simulacaoModeloY(void *arg) {
  DadosCompartilhados *dados = (DadosCompartilhados *)arg;
  FILE *arquivoLog = inicializarRegistro("registro_ymy.txt", "Tempo, ymy");
  if (!arquivoLog) {
    return NULL;
  }
...
void *gerarReferencia(void *arg) {
  DadosCompartilhados *dados = (DadosCompartilhados *)arg;
  FILE *arquivoLog = inicializarRegistro("registro_ref.txt", "Tempo, Xref, Yref");
  if (!arquivoLog) {
